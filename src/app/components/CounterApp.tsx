...
import { BrowserProvider, Contract, JsonRpcProvider, WebSocketProvider, type ContractTransactionResponse } from "ethers";
...
export default function CounterApp() {
  ...
  const [walletReady, setWalletReady] = useState<"unknown" | "none" | "ok">("unknown");
  ...
  const readOnlyContract = useMemo(() => {
    ...
    return new Contract(counterContract.address, counterContract.abi, readProvider);
  }, [readProvider]);

  const wsContract = useMemo(() => {
    if (!wsProvider) return null;
    return new Contract(counterContract.address, counterContract.abi, wsProvider);
  }, [wsProvider]);
  ...
  async function runTx(
    ...
    try {
      ...
      const c = new Contract(counterContract.address, counterContract.abi, signer);
      const tx = await fn(c);
      await tx.wait();
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-col gap-8 px-4 py-16">
      <header className="space-y-2 text-center">
        ...
        <div className="mx-auto grid w-full max-w-md gap-1 text-left text-sm">
          <div className="flex items-center justify-between gap-3">
            <span className="text-zinc-500 dark:text-zinc-400">컨트랙트 주소</span>
            <a
          ...
          <div className="flex items-center justify-between gap-3">
            <span className="text-zinc-500 dark:text-zinc-400">이벤트 구독</span>
            <span className="font-mono text-zinc-800 dark:text-zinc-200">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="font-mono text-zinc-800 dark:text-zinc-200">ethers.js</span>
          </div>
        </div>
      </header>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white/80 p-8 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/80">
        ...
        <div className="flex flex-col gap-3 border-t border-zinc-100 pt-6 dark:border-zinc-800">
          {walletReady === "unknown" ? (
            ...
            <p className="text-center text-sm text-amber-700 dark:text-amber-400">
              브라우저에 지갑 확장 프로그램을 설치하면 트랜잭션을 보낼 수 있습니다. 읽기 전용 값은 위에 표시됩니다.
            </p>
          ) : !account ? (
            <button
              type="button"
              ...
              {!chainOk ? (
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    type="button"
                    disabled={!canTx}
                    onClick={() => void runTx("증가", (c) => c.incrementCounter())}
                  ...
                  <button
                    type="button"
                    disabled={!canTx}
                    onClick={() => void runTx("초기화", (c) => c.resetCounter())}
                  ...
                  <button
                    type="button"
                    disabled={!canTx}
                    onClick={() => void runTx("감소", (c) => c.decrementCounter())}
        ...
        {status === "pending" && (
          <p className="mt-2 text-center text-xs text-zinc-500">트랜잭션 확인 대기 중…</p>
        )}
      </section>
    </div>
  );
